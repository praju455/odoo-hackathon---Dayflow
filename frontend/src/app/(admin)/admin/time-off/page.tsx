  ...
  return (
    <div className="space-y-6">
      ...
      <div className="inline-flex rounded-lg border border-gray-200 bg-gray-100 p-1">
        <button onClick={() => setTab("requests")} className={`rounded-md px-4 py-2 text-sm font-medium ${tab === "requests" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500"}`}>Time Off ({pendingCount})</button>
        <button onClick={() => setTab("allocations")} className={`rounded-md px-4 py-2 text-sm font-medium ${tab === "allocations" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500"}`}>Allocation</button>
      </div>
        ...
        {tab === "requests" ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-gray-600">
              <thead className="border-b border-gray-100 bg-gray-50 text-xs uppercase text-gray-500"><tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">Loading requests...</td></tr>
                  ...
                  : requests.map((request) => <tr key={request.id}>
                    <td className="px-6 py-4"><div className="flex items-center gap-3"><Avatar name={request.user?.name ?? "Employee"} size="sm" /><div><p className="font-medium text-gray-900">{request.user?.name ?? "Employee"}</p><p className="text-xs text-gray-400">{request.user?.department ?? "No department"}</p></div></div></td>
                    ...
                    <td className="px-6 py-4 text-right">{request.status === "PENDING" ? <div className="flex justify-end gap-2"><button onClick={() => setDecision({ request, status: "REJECTED" })} className="rounded-md border border-gray-300 px-3 py-1.5 font-medium text-gray-700 hover:bg-gray-50">Reject</button><button onClick={() => setDecision({ request, status: "APPROVED" })} className="rounded-md bg-indigo-600 px-3 py-1.5 font-medium text-white hover:bg-indigo-500">Approve</button></div> : <span className="text-xs text-gray-400">Reviewed</span>}</td>
                  </tr>)}
              </tbody>
            </table>
          </div>
          ...
          <div className="overflow-x-auto"><table className="w-full text-left text-sm text-gray-600">
            <thead className="border-b border-gray-100 bg-gray-50 text-xs uppercase text-gray-500"><tr><th className="px-6 py-4 font-medium">Employee</th><th className="px-6 py-4 font-medium">Paid</th><th className="px-6 py-4 font-medium">Sick</th><th className="px-6 py-4 font-medium">Unpaid</th></tr></thead>
            <tbody className="divide-y divide-gray-100">
      ...
      {decision && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <h2 className="text-lg font-semibold text-gray-900">{decision.status === "APPROVED" ? "Approve" : "Reject"} leave request</h2>
